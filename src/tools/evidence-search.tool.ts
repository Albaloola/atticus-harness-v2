...
export class EvidenceSearchTool implements Tool<{ query: string; topK?: number }, unknown[]> {
  ...
  readonly description = 'Search indexed evidence using full-text search. Returns ranked matches with snippets.';
  ...
  async call(args: { query: string; topK?: number }, context: ToolUseContext): Promise<ToolResult<unknown[]>> {
    ...
    try {
      ...
      const output = results.length === 0
        ...
        : `Found ${results.length} results:\n${results.map((r, i) => `  ${i + 1}. [${r.evidenceId}] (score: ${r.score.toFixed(2)})\n     ${r.snippet}`).join('\n')}`;
