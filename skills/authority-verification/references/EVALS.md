...
- Uses the skill, Runs `verify` first, Retrieves source text only after verification, Avoids branded citator terminology
...
- Does not assume the case exists, Surfaces `not_found` or ambiguity clearly, Does not label it "good law"
...
- Uses `citationsFromUrl` or an equivalent extraction step, Verifies important citations individually, Distinguishes verified citation existence from manual treatment review, Produces a structured research trail
