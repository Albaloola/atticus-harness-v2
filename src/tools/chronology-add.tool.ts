...
export function createChronologyAddTool(): ToolDefinition<ChronologyAddInput> {
  return {
    id: 'chronology_add',
    name: 'chronology_add',
    description: 'Add a new event to the global living chronology for the current matter. Use this to construct a timeline incrementally as you discover facts. Use ISO 8601 date formats.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    call: async (input, context: ToolUseContext) => {
      ...
      try {
        const event = addChronologyEvent({
          ...
          status: input.status,
          sourceTeam: context.getConfig().role ?? 'unknown',
        });
