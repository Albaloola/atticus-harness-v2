...
program.commands.find((command) => command.name() === 'export')!
  .addCommand(
    new Command('documents')
      ...
      .argument('<matter-name>', 'Matter name')
      .option('--objective <text>', 'Output request for the document output pipeline, for example an exact form or a specific document kind')
      .option('--allow-remote-forms', 'Allow the document output pipeline to download missing official ScotCourts forms')
...
program
  ...
  .addCommand(
    new Command('manage')
      ...
      .argument('<matter-name>', 'Matter name')
      .argument('<instruction>', 'agent/operator instruction')
      .option('--type <type>', 'email, communication, task, case_management, draft, or report')
      .option('--source <source>', 'Instruction source', 'agent')
