...
export class ChronologyUpdateTool implements Tool<ChronologyUpdateInput, any> {
  readonly name = 'chronology_update';
  readonly description = 'Update an existing event in the global living chronology. Useful for correcting dates, marking events as disputed, or linking events to newly discovered evidence or issues.';
  ...
  async call(input: ChronologyUpdateInput, context: ToolUseContext): Promise<ToolResult<any>> {
    ...
    try {
      const event = updateChronologyEvent(context.matterName, input.id, {
        ...
        status: input.status,
        sourceTeam: context.getConfig().role as string | undefined,
      });
...
export function createChronologyUpdateTool(): Tool<ChronologyUpdateInput, any> {
  return new ChronologyUpdateTool();
}
