...
- Deadline, fee, and extension verified from official source, Audit/review threshold applied correctly for revenue level, Oath language matches statutory requirements or flagged `[VERIFY]`
...
- Use official state/legal sources only, never infer legal obligations, Reconcile or flag any cross-table inconsistencies, Preserve board/officer identities and compensation exactly as sourced, Include jurisdiction caveat when state rules vary by year or filing type, Mark uncertain legal statements `[VERIFY]`
