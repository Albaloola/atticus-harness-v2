...
- Read the top 3 to 10 chunks per query. Cite everything the memo will rely on by object name + page.
...
Don't brute-force read every file. Drive the search with focused queries, pull the 3 to 10 most relevant chunks per query, follow up when hits are load-bearing, route when a cluster warrants a sibling skill.
...
- **20 to 200 files:** one pass through the search playbook, one memo. ~1 hour.
- **200 to 2,000 files:** expect 40 to 80 targeted searches across the dimensions, checkpoint notes as the agent goes. Multi-hour run.
