...
export function createDelegateTaskTool(): ToolDefinition<DelegateTaskInput> {
  return {
    id: 'delegate_task',
    name: 'delegate_task',
    description: 'Delegate a specific task to another legal team. Use this when you are missing information, need specialized advice, or require peer review before proceeding.',
    inputSchema: {
      type: 'object',
      properties: {
        targetTeam: {
      ...
      try {
        const task = createTask({
          ...
          data: {
            ...
            requestedBy: context.getConfig().role ?? 'LeadCounsel'
          }
        });
        ...
        const loop = new QueryLoop({
          model: resolvedConfig.providerPolicy?.defaultModel || 'deepseek/deepseek-v4-pro', // Fallback to a strong model
          maxTokens: 8192,
          maxTurns: 30,
          systemPrompt,
        ...
        // 3. Execute Sub-Agent
        updateTask(context.matterName, task.id, { status: 'running' });
