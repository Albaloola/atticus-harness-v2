...
export class CaseCitationAddTool implements Tool<CaseCitationAddInput, any> {
  readonly name = 'case_citation_add';
  readonly description = 'Add a referenced legal case citation (e.g. from legal research) along with a relevance rating, key quotes, and rationale, so that subsequent drafting subagents can cite and recall it efficiently.';
  ...
  async call(input: CaseCitationAddInput, context: ToolUseContext): Promise<ToolResult<any>> {
...
export function createCaseCitationAddTool(): Tool<CaseCitationAddInput, any> {
  return new CaseCitationAddTool();
}
